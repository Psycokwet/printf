...
/*   Updated: 2020/06/30 13:22:00 by scarboni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int set_flag(const char *code, t_data *datas){
    ...
    while(--i >= 0){
        if(ft_strncmp(FLAGS[i].code.str, code, FLAGS[i].code.size)){
...
int set_convert(const char *code, t_data *datas){
    ...
    while(--i >= 0){
        if(ft_strncmp(CONVERTS[i].code.str, code, CONVERTS[i].code.size)){
            CONVERTS[i].print(datas);
            printf("Found convert namely :%s\n", CONVERTS[i].code.str);
            return 1;
        }
    }
    return 0;
