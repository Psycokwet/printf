...
/*   Updated: 2020/07/21 21:48:07 by scarboni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void set_width_precision_x_up_x(t_data *datas)
{
    ...
    if(datas->active_flags & FT_PF_NEG_PRECISION && datas->active_flags & FT_PF_FLAG_FIELD_WIDTH && datas->active_flags & FT_PF_FLAG_DIESE ){
        datas->precision = datas->field_width;
        datas->active_flags -= FT_PF_FLAG_FIELD_WIDTH;
        datas->active_flags |= FT_PF_FLAG_PRECISION;
    }
    printf("W:%d::P:%d::L:%zu::LESS:%d::WIDTH:%d\n",datas->field_width,datas->precision, datas->len, datas->active_flags & FT_PF_FLAG_LESS ? 1: 0, datas->active_flags & FT_PF_FLAG_FIELD_WIDTH ? 1: 0);
